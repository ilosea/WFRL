    ...
    def update(self, dt, t):
        ...
        if self._next_update <= t:
            ...
            for frame in range(skipped_frames):
                for spr in self.sprites():
                    spr.update(dt, t)
        self.sum.append(pygame.time.get_ticks()-start_time)
        print self.sum[-1]
...
def main():
    ...
    print "len:",len(anims.sum), "sum:", sum(anims.sum), "average:",sum(anims.sum)/float(len(anims.sum))
